...
function wsFromMatrix(matrix) {
  ...
  matrix.forEach((row, r) => row.forEach((cell, c) => {
    ...
    if (cell && typeof cell === "object" && "f" in cell) ws[a] = {
      t: cell.t || "n",
      f: cell.f,
...
function LayoutZone({
  ...
  const downloadAll = async () => {
    ...
    setDownloadingAll(true);
    try {
      await bulkDownloadFiles(list, `${brand ? brand.name : ""}Layout圖_${month}`, toast);
    } finally {
      setDownloadingAll(false);
    }
  };
  ...
  const exportBillingOsheng = () => {
    ...
    ORG.forEach(o => {
      ...
      o.depts.forEach(d => {
        ...
        deptRowIdx[d] = R;
        MI.push([d, {
          f: `COUNTIF('請款明細(彙總)'!$J:$J,A${R})`,
          v: cntOf(d)
        }, {
          f: `SUMIF('請款明細(彙總)'!$J:$J,A${R},'請款明細(彙總)'!$L:$L)`,
          v: sumOf(d)
        }]);
      });
      ...
      MI.push([o.div, {
        f: `SUM(B${startR}:B${endR})`,
        v: subCnt
      }, {
        f: `SUM(C${startR}:C${endR})`,
        v: subSum
      }]);
    });
    // 總計 = 各部小計加總
    ...
    MI.push(["總計", {
      f: `${subRows.map(r => `B${r}`).join("+")}`,
      v: viewRows.length
    }, {
      f: `${subRows.map(r => `C${r}`).join("+")}`,
      v: grand
    }]);
    MI.push(["5% 稅金", "", {
      f: `ROUND(C${totalR}*0.05,0)`,
      v: Math.round(grand * 0.05)
    }]);
    MI.push(["含稅總計", "", {
      f: `C${totalR}+C${totalR + 1}`,
      v: grand + Math.round(grand * 0.05)
    }]);
...
function MaintainZone({
  ...
  return /*#__PURE__*/React.createElement(SectionCard, {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "text-xs text-slate-400"
    }, "歐聖請款＝MAX(實盤件數×單價, 最低收費)＋(總倉別-1)×倉別加計；件數不足最低收費者標記「最低收費」。"), showFees && /*#__PURE__*/React.createElement("p", {
      className: "text-xs text-slate-400"
